...
%% ANOVA: drop + delay + dV (continuous) + choice
factornames = {'drop', 'delay', 'DV', 'choice'};
anv = {};
for i = 1:2
    d = cue{i};
    anv{i} = W.anovan_slidingwindow_combinedgames(d, factornames, 'continuous', [1 2 3], 'is_normalize', true);
end
W.save('../../TempData/result/anv_dropC_delayC_DV_choice', 'anv', anv);
%% Figure - explained variance over time
anv = W.load('../../TempData/result/anv_dropC_delayC_DV_choice');
SW_fig(plt, anv{1}, anv{2}, 'axes', 'ax_slidingwindow_ANOVA','ax_slidingwindow_ANOVA', ...
    'ax_slidingwindow_ANOVA_omega','ax_slidingwindow_ANOVA_omega', ...
     'data_axes', 1, 2, 1, 2, ...
     'varargin_axes', {'plottype', 'shade', ...
     ...
     'legloc', 'NE', 'xlim', [-500, 1000], 'ylim', [-0.001 0.035], 'ytick', [-0.05:0.01:0.05]}, ...
     'ABC_axes', 'ABCD', 'nx', 2, 'ny', 2, ...
     'varargin_fig', 'is_title', 'all', 'savename', 'ANOVA_dropC_delayC_DV_choice');
...
plt.figure(2,3, 'gapW_custom', [0 0 0 7], 'is_title', 'all');
tlts = {'drop', 'delay', 'DV'};
for ai = 1:2
    for i = 1:3
        plt.ax(ai,i);
...
plt.figure(2,2, 'gapW_custom', [0.5 0 0.5], 'is_title', 1);
names = ["drop", "delay", "DV"];
iii = {[1 3], [2 3], [1 2]};
for ai = 1:2
    for i = 1:2
